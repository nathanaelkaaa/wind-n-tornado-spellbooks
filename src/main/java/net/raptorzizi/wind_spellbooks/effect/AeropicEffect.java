...
import io.redspace.ironsspellbooks.mixin.LivingEntityAccessor; // IMPORTANT
...
public class AeropicEffect extends MagicMobEffect {
    ...
    public void onEffectAdded(LivingEntity entity, int amplifier) {
        ...
        if (entity instanceof LivingEntityAccessor accessor) {
            accessor.setLivingEntityFlagInvoker(4, true);
        }

        if (entity instanceof ServerPlayer serverPlayer) {
            serverPlayer.getPersistentData().remove("aeropic_hit_confirmed");
            boolean alreadyExists = serverPlayer.getPersistentData().contains("aeropic_recasts_remaining");
            if (!alreadyExists) {
                var spell = ModSpellRegistry.ACROBATICS_SPELL.get();
                int maxRecasts = spell.getRecastCount(amplifier, serverPlayer) - 1;
    ...
    public void onEffectRemoved(LivingEntity entity, int amplifier) {
        ...
        if (entity instanceof LivingEntityAccessor accessor) {
            accessor.setLivingEntityFlagInvoker(4, false);
        }

        if (entity instanceof ServerPlayer serverPlayer) {
            MagicData magicData = MagicData.getPlayerMagicData(serverPlayer);
            var spell = ModSpellRegistry.ACROBATICS_SPELL.get();
            boolean hitConfirmed = serverPlayer.getPersistentData().getBoolean("aeropic_hit_confirmed");
            serverPlayer.getPersistentData().remove("aeropic_hit_confirmed");
    ...
    public void applyEffectTick(LivingEntity livingEntity, int amplifier) {
        ...
        for (Entity entity : level.getEntities(livingEntity, livingEntity.getBoundingBox().inflate(explosionRadius))) {
            ...
            if (distSqr < explosionRadiusSqr && entity.canBeHitByProjectile() && Utils.hasLineOfSight(level, losPoint, entity.getBoundingBox().getCenter(), true)) {
                float damage = (float) (amplifier * (1 - distSqr / explosionRadiusSqr) * 0.5);
                DamageSources.applyDamage(entity, damage, ModSpellRegistry.ACROBATICS_SPELL.get().getDamageSource(livingEntity));
            }
        }
        ...
        if (livingEntity instanceof ServerPlayer serverPlayer) {
            MagicData magicData = MagicData.getPlayerMagicData(serverPlayer);
            var spell = ModSpellRegistry.ACROBATICS_SPELL.get();
            int recastsRemaining = serverPlayer.getPersistentData().getInt("aeropic_recasts_remaining");

            if (amplifier > 1 && recastsRemaining > 0) {
                serverPlayer.getPersistentData().putInt("aeropic_recasts_remaining", recastsRemaining - 1);
                serverPlayer.getPersistentData().putBoolean("aeropic_hit_confirmed", true);

                if (!magicData.getPlayerRecasts().hasRecastForSpell(spell.getSpellId())) {
                    magicData.getPlayerRecasts().addRecast(new RecastInstance(spell.getSpellId(), amplifier, spell.getRecastCount(amplifier, serverPlayer), 60, CastSource.SPELLBOOK, null), magicData);
                }
            }
        }
        ...
        level.playSound(null, x, y, z, SoundEvents.EVOKER_CAST_SPELL, livingEntity.getSoundSource(), 4, 1.2f);
    }
